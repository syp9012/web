...
function sel(){
	...
	if(email.value==""){		//'직접입력'을 선택한 경우
		domain.readOnly=false;	//읽기전용 옵션 해제
		domain.value="";
		domain.focus();
	}else{						//'도메인명'을 선택한 경우
		domain.value=email.value;
		domain.readOnly=true;	//읽기만 가능  > readOnly 대문자로 기재할것.
	}
}
// 유효성 검사
function check(){
	...
	if(isNaN(tel3.value)){
		alert("전화번호 끝자리에 숫자를 입력하세요.");
		tel3.value="";
		tel3.focus();
		return false;
	}
	if(phone1.value == ""){
		alert("핸드폰번호 앞자리를 선택 하세요.");
		phone1.focus();
		return false;	
	}
	if(phone2.value==""){
	...
	if(isNaN(phone2.value)){
		alert("핸드폰번호 중간자리에 숫자를 입력하세요.");
		phone2.value="";
		phone2.focus();
		return false;
	}
	if(phone3.value==""){
		alert("핸드폰번호 끝자리를 입력 하세요.");
		phone3.focus();
		return false;	
	}
	if(isNaN(phone3.value)){
		alert("핸드폰번호 끝자리에 숫자를 입력하세요.");
		phone3.value="";
		phone3.focus();
		return false;
	}
